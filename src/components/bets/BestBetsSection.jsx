...
const BreakdownPopover = ({ bet, onClose }) => {
  ...
  return (
    <div className="absolute right-0 sm:right-0 top-full mt-2 z-50 w-[calc(100vw-3rem)] sm:w-72 bg-[#12151c] border border-white/10 rounded-xl p-4 shadow-2xl space-y-3 text-xs">
...
const BestBetRow = ({ bet }) => {
  ...
  return (
    <div className="relative py-2.5 px-3 sm:py-3 sm:px-4 bg-white/[0.02] rounded-lg border border-white/5 hover:border-white/10 transition-colors">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
          ...
          {bet.headshot && (
            <img
              src={bet.headshot}
              alt=""
              className="w-7 h-7 sm:w-9 sm:h-9 rounded-full flex-shrink-0 bg-slate-800"
            />
          )}
        ...
        {/* Desktop: full prop details inline */}
        ...
        {/* Mobile: compact right-side info */}
        <div className="flex sm:hidden items-center gap-2 flex-shrink-0 ml-2">
          <div className="text-right">
            <div className="flex items-center gap-1 justify-end">
              <span className="text-[10px] font-black text-slate-500 uppercase">{PROP_LABELS[bet.propType]}</span>
              {bet.current != null ? (
                <span className="text-xs font-bold tabular-nums">
                  <span className="text-blue-400">{bet.current}</span>
                  <span className="text-slate-600">/</span>
                  <span className="text-white">{bet.line}</span>
                </span>
              ) : (
                <span className="text-sm font-bold text-white tabular-nums">{bet.line}</span>
              )}
            </div>
            <div className="flex items-center gap-1.5 justify-end">
              <span className={`text-[11px] font-bold tabular-nums ${isOver ? 'text-emerald-400' : 'text-red-400'}`}>
                {isOver ? `o${mockOdds.formatOdds(bet.overOdds)}` : `u${mockOdds.formatOdds(bet.underOdds)}`}
              </span>
              <span className={`text-[10px] font-bold uppercase ${isOver ? 'text-emerald-500/70' : 'text-red-400/70'}`}>
                {strength}
              </span>
            </div>
          </div>
...
export const BestBetsSection = ({ bets, loading }) => {
  ...
  if (loading) {
    return (
      <div className="bg-white/[0.02] border border-white/5 rounded-xl p-3 sm:p-5 space-y-3">
  ...
  return (
    <div className="bg-white/[0.02] border border-white/5 rounded-xl p-3 sm:p-5 space-y-3">
      <h2 className="text-xs font-black text-slate-500 uppercase tracking-wider sm:tracking-[0.2em] flex items-center gap-2">
        Best Bets
        <span className="text-[8px] bg-amber-500/10 text-amber-400 px-1.5 py-0.5 rounded font-bold">
          Top Signals
        </span>
